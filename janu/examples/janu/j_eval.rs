...
async fn main() {
    ...
    while let Some(get_request) = get_stream.next().await {
        ...
        // following selectors:
        // - "/janu/example/eval" : no properties are set, a default value is used for the name
        // - "/janu/example/eval?(name=Bob)" : "Bob" is used for the name
        // - "/janu/example/eval?(name=/janu/example/name)" : the Eval function does a GET
        //      on "/janu/example/name" an uses the 1st result for the name
...
fn parse_args() -> (Properties, String) {
    let args = App::new("janu eval example")
        .arg(
