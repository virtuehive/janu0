...
use janu::net::plugins::*;
use janu::net::runtime::{AdminSpace, Runtime};
use janu_util::properties::config::*;
use janu_util::properties::Properties;
use janu_util::LibLoader;
...
fn main() {
    task::block_on(async {
        ...
        let app = App::new("The janu router")
            .version(GIT_VERSION)
            ...
            .arg(Arg::from_usage(
                "-i, --id=[hex_string] \
            'The identifier (as an hexadecimal string - e.g.: 0A0B23...) that janud must use. \
            ...
            .arg(Arg::from_usage(
                "--plugin-nolookup \
             'When set, janud will not look for plugins nor try to load any plugin except the \
             ones explicitely configured with -P or --plugin.'",
            ))
            .arg(Arg::from_usage(&plugin_search_dir_usage).conflicts_with("plugin-nolookup"))
            .arg(Arg::from_usage(
                "--no-timestamp \
             'By default janud adds a HLC-generated Timestamp to each routed Data if there isn't already one. \
             This option disables this feature.'",
            )).arg(Arg::from_usage(
                "--no-multicast-scouting \
             'By default janud replies to multicast scouting messages for being discovered by peers and clients. 
              This option disables this feature.'",
        ));
