...
use crate::{
    zready, Path, PathExpr, Selector, Timestamp, Value, ZError, ZErrorKind, ZResult, Janu,
};
...
/// A Workspace to operate on janu.
///
...
impl Workspace<'_> {
    pub(crate) fn new(
        ...
        zready(Ok(Workspace { janu, prefix }))
    }

    /// Returns the prefix that was used to create this Workspace (calling [`Janu::workspace()`]).
    pub fn prefix(&self) -> &Option<Path> {
        &self.prefix
    }

    /// Returns the janu-net [`Session`] used by this workspace.
    ...
    fn canonicalize(&self, path: &str) -> ZResult<String> {
        ...
        if abs_path.starts_with(LOCAL_ROUTER_PREFIX) {
            match self.janu.router_pid().wait() {
                Some(pid) => Ok(format!(
                    "/@/router/{}{}",
                    pid,
    ...
    /// Delete a [`Path`] and its [`Value`] from janu.  
    /// The corresponding [`Change`] will be received by all matching subscribers and all matching storages.
    /// Note that the [`Path`] can be absolute or relative to this Workspace.
    ///
    /// # Examples
    ...
    /// Get a selection of [`Path`]/[`Value`] from janu.  
    ...
    /// Subscribe to changes for a selection of [`Path`]/[`Value`] (specified via a [`Selector`]) from janu.  
...
/// at which the path/value has been put into janu.
#[derive(Debug)]
pub struct Data {
    pub path: Path,
    pub value: Value,
...
impl Change {
    ...
    /// Convert this [`Change`] into a [`Sample`] to be sent via janu-net.
