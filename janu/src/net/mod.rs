...
/// # Arguments
///
/// * `what` - The kind of janu process to scout for
...
/// Open a janu-net [Session](Session).
///
/// # Arguments
///
/// * `config` - The [ConfigProperties](ConfigProperties) for the janu-net session
///
/// # Examples
/// ```
/// # async_std::task::block_on(async {
...
pub fn open(config: ConfigProperties) -> impl ZFuture<Output = ZResult<Session>> {
    debug!("Janu Rust API {}", GIT_VERSION);
    debug!("Config: {:?}", &config);
    Session::new(config)
}
