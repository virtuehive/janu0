...
use super::protocol::proto::{
    Data, Declaration, Declare, LinkStateList, Pull, Query, Unit, JanuBody, JanuMessage,
};
...
impl<P: 'static + Primitives> TransportPeerEventHandler for DeMux<P> {
    fn handle_message(&self, msg: JanuMessage) -> ZResult<()> {
        match msg.body {
            JanuBody::Declare(Declare { declarations, .. }) => {
            ...
            JanuBody::Unit(Unit { reply_context, .. }) => {
            ...
            JanuBody::LinkStateList(LinkStateList { .. }) => {}
        }
