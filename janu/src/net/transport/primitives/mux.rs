...
impl Primitives for Mux {
    fn decl_resource(&self, rid: ZInt, reskey: &ResKey) {
        ...
        let _ = self
            .handler
            .handle_message(JanuMessage::make_declare(decls, None, None));
    }
    ...
    fn decl_subscriber(
        ...
        let _ =
            self.handler
                .handle_message(JanuMessage::make_declare(decls, routing_context, None));
    }
    ...
    fn send_data(
        ...
        let _ = self.handler.handle_message(JanuMessage::make_data(
            reskey.clone(),
            payload,
            channel,
            cogestion_control,
    ...
    fn send_query(
        ...
        let _ = self.handler.handle_message(JanuMessage::make_query(
            reskey.clone(),
            predicate.to_string(),
            qid,
            target_opt,
    ...
    fn send_reply_final(&self, qid: ZInt) {
        let _ = self.handler.handle_message(JanuMessage::make_unit(
            zmsg::default_channel::REPLY,
            zmsg::default_congestion_control::REPLY,
    ...
    fn send_pull(
        ...
        let _ = self.handler.handle_message(JanuMessage::make_pull(
            is_final,
            reskey.clone(),
            pull_id,
            *max_samples,
