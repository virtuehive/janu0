...
use janu_util::sync::{Condition as AsyncCondvar, ConditionWaiter as AsyncCondvarWaiter};
use janu_util::zlock;
...
impl TransmissionPipeline {
    ...
    #[inline]
    pub(crate) fn push_janu_message(&self, mut message: JanuMessage) -> bool {
        ...
        macro_rules! zserialize {
            () => {
                ...
                if batch.serialize_janu_message(&mut message, mp, &mut ch_guard.sn) {
                    self.bytes_in[priority].store(batch.len(), Ordering::Release);
                    self.cond_canpull.notify_one();
                    return true;
                }
        ...
        self.fragment_janu_message(message, priority, ch_guard, in_guard)
    }

    fn fragment_janu_message(
        &self,
        mut message: JanuMessage,
        priority: usize,
        ...
        fragbuf.clear();
        fragbuf.write_janu_message(&mut message);
        ...
        while to_write > 0 {
            ...
            if written != 0 {
                ...
                log::warn!(
                    "Janu message dropped because it can not be fragmented: {:?}",
                    message
                );
                break;
            }
...
mod tests {
    ...
    use crate::net::protocol::proto::{Frame, FramePayload, TransportBody, JanuMessage};
    ...
    fn tx_pipeline_flow() {
        fn schedule(queue: Arc<TransmissionPipeline>, num_msg: usize, payload_size: usize) {
            ...
            for _ in 0..num_msg {
                queue.push_janu_message(message.clone());
            }
        }
    ...
    fn tx_pipeline_blocking() {
        fn schedule(queue: Arc<TransmissionPipeline>, counter: Arc<AtomicUsize>, id: usize) {
            ...
            // to half of the batch in such a way the serialized janu message
            ...
            loop {
                ...
                for size in payload_sizes.iter() {
                    ...
                    while start.elapsed() < duration {
                        c_pipeline.push_janu_message(message.clone());
                    }
                }
            }
        });
