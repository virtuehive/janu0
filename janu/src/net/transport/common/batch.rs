...
use super::protocol::proto::{TransportMessage, JanuMessage};
...
/// that is used to serialize [`TransportMessage`][TransportMessage] and [`JanuMessage`][JanuMessage].
///
/// [`TransportMessage`][TransportMessage] are always serialized on the batch as they are, while
/// [`JanuMessage`][JanuMessage] are always serializaed on the batch as part of a [`TransportMessage`]
...
/// its payload is either (i) one or more complete [`JanuMessage`][JanuMessage] or (ii) a fragment of a
/// a [`JanuMessage`][JanuMessage].
///
/// As an example, the content of the [`SerializationBatch`][SerializationBatch] in memory could be:
///
/// | Keep Alive | Frame Reliable<Janu Message, Janu Message> | Frame Best Effort<Janu Message Fragment> |
///
#[derive(Clone, Debug)]
...
impl SerializationBatch {
    ...
    /// Try to serialize a [`JanuMessage`][JanuMessage] on the [`SerializationBatch`][SerializationBatch].
    ///
    /// # Arguments
    /// * `message` - The [`JanuMessage`][JanuMessage] to serialize.
    ///
    pub(crate) fn serialize_janu_message(
        &mut self,
        message: &mut JanuMessage,
        priority: Priority,
        ...
        let res = if let Some(frame) = new_frame {
            ...
            // Serialize the new frame and the janu message
            let res = self.buffer.write_frame_header(
                ...
                None,
                None,
            ) && self.buffer.write_janu_message(message);
    ...
    /// Gets a serialized [`JanuMessage`][JanuMessage] and fragment it on the [`SerializationBatch`][SerializationBatch].
    ///
    /// # Arguments
    /// * `reliable` - The serialized [`JanuMessage`][JanuMessage] is for the reliable or the best effort channel.
    ///
    /// * `sn` - The reliable/best effort sequence number of the new [`JanuMessage`][JanuMessage] fragment.
    ///
    /// * `to_fragment` - The buffer containing the serialized [`JanuMessage`][JanuMessage] that requires fragmentation.
    ///
    /// * `to_write` - The amount of bytes that still need to be fragmented.
    ///
    pub(crate) fn serialize_janu_fragment(
        &mut self,
...
mod tests {
    ...
    use crate::net::protocol::proto::{
        Frame, FramePayload, TransportBody, TransportMessage, JanuMessage,
    };
    ...
    fn serialize_no_fragmentation(batch_size: u16, payload_size: usize) {
        for is_streamed in [false, true].iter() {
            ...
            let mut zmsgs_in: Vec<JanuMessage> = vec![];
            let mut reliable = true;
            let mut dropping = true;
            loop {
                // Insert a transport message every 3 JanuMessage
                if zmsgs_in.len() % 3 == 0 {
                ...
                // Serialize the JanuMessage
                let res = batch.serialize_janu_message(&mut msg, channel.priority, &mut sn_gen);
                if !res {
                    batch.write_len();
                    assert!(!zmsgs_in.is_empty());
                    break;
            ...
            let mut zmsgs_out: Vec<JanuMessage> = vec![];
    ...
    fn serialize_fragmentation(batch_size: u16, payload_size: usize) {
        for is_streamed in [false, true].iter() {
            ...
            for reliability in [Reliability::BestEffort, Reliability::Reliable].iter() {
                ...
                let reply_context = None;
                let attachment = None;
                let mut msg_in = JanuMessage::make_data(
                    key,
                    payload,
                    channel,
                    congestion_control,
                ...
                wbuf.write_janu_message(&mut msg_in);
                ...
                while to_write > 0 {
                    // Create the serialization batch
                    let mut batch = SerializationBatch::new(batch_size, *is_streamed);
                    let written = batch.serialize_janu_fragment(
                ...
                let msg_out = fragments.read_janu_message(*reliability);
