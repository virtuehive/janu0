...
impl Runtime {
    ...
    async fn bind_listeners(&self, listeners: &[Locator]) -> ZResult<()> {
        ...
        for locator in self.manager().get_locators() {
            log::info!("janud can be reached on {}", locator);
        }
        Ok(())
    }
    ...
    pub fn get_interfaces(names: &str) -> Vec<IpAddr> {
        if names == "auto" {
            let ifaces = janu_util::net::get_multicast_interfaces();
            if ifaces.is_empty() {
                log::warn!(
            ...
            names
                ...
                .filter_map(|name| match name.trim().parse::<IpAddr>() {
                    Ok(addr) => Some(addr),
                    Err(_) => match janu_util::net::get_interface(name.trim()) {
    ...
    pub async fn bind_mcast_port(sockaddr: &SocketAddr, ifaces: &[IpAddr]) -> ZResult<UdpSocket> {
        ...
        log::info!("janud listening scout messages on {}", sockaddr);
