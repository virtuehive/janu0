...
pub mod zmsg {
    ...
    // Janu message IDs -- Re-export of some of the Inner Message IDs
    pub mod id {
        use super::imsg;
    ...
    // Default reliability for each Janu Message
    ...
    // Default congestion control for each Janu Message
...
///
/// The Attachment can decorate any message (i.e., TransportMessage and JanuMessage) and it allows to
/// append to the message any additional information. Since the information contained in the
/// Attachement is relevant only to the layer that provided them (e.g., Transport, Janu, User) it
...
/// informations related to the priority of the frame/janu message.
///
///  7 6 5 4 3 2 1 0
/// +-+-+-+-+-+-+-+-+
/// | ID  |  Prio   |
...
impl fmt::Debug for JanuMessage {
    #[cfg(feature = "stats")]
...
impl fmt::Display for JanuMessage {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl JanuMessage {
    pub fn make_declare(
        ...
        JanuMessage {
            body: JanuBody::Declare(Declare { declarations }),
    ...
    pub fn make_link_state_list(
        ...
        JanuMessage {
            body: JanuBody::LinkStateList(LinkStateList { link_states }),
    ...
    pub fn is_droppable(&self) -> bool {
        ...
        let cc = match &self.body {
            JanuBody::Data(data) => data.congestion_control,
            JanuBody::Unit(unit) => unit.congestion_control,
            JanuBody::Declare(_) => zmsg::default_congestion_control::DECLARE,
            JanuBody::Pull(_) => zmsg::default_congestion_control::PULL,
            JanuBody::Query(_) => zmsg::default_congestion_control::QUERY,
            JanuBody::LinkStateList(_) => zmsg::default_congestion_control::LINK_STATE_LIST,
        };
...
/// +---------------+
///
/// - if K==0 then close the whole janu transport.
...
/// ~      SN       ~
/// +---------------+
/// ~  FramePayload ~ -- if F==1 then the payload is a fragment of a single Janu Message, a list of complete Janu Messages otherwise.
/// +---------------+
///
...
pub enum FramePayload {
    ...
    /// ~  JanuMessage ~
    /// +---------------+
    /// ~      ...      ~ - Additional complete Janu messages.
    /// +---------------+
    ///
    /// NOTE: A batched Frame must contain at least one complete Janu message.
    ///       There is no upper limit to the number of Janu messages that can
    ///       be batched together in the same frame.
    /// ```
    Messages { messages: Vec<JanuMessage> },
}

// Janu messages at janu-transport level
#[derive(Debug, Clone, PartialEq)]
