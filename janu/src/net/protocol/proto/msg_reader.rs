...
impl ZBuf {
    ...
    fn read_frame(&mut self, header: u8, priority: Priority) -> Option<TransportBody> {
        ...
        let payload = if imsg::has_flag(header, tmsg::flag::F) {
            ...
            let mut messages: Vec<JanuMessage> = Vec::with_capacity(1);
            while self.can_read() {
                let pos = self.get_pos();
                if let Some(msg) = self.read_janu_message(reliability) {
                    messages.push(msg);
    ...
    pub fn read_janu_message(&mut self, reliability: Reliability) -> Option<JanuMessage> {
        use super::zmsg::id::*;
        ...
        let body = loop {
            ...
            match imsg::mid(header) {
                ...
                unknown => {
                    log::trace!("Janu message with unknown ID: {}", unknown);
                    return None;
                }
            }
        };
    ...
    #[inline(always)]
    fn read_data(&mut self, header: u8, reply_context: Option<ReplyContext>) -> Option<JanuBody> {
    ...
    fn read_unit(&mut self, header: u8, reply_context: Option<ReplyContext>) -> Option<JanuBody> {
        let congestion_control = if imsg::has_flag(header, zmsg::flag::D) {
            CongestionControl::Drop
        } else {
            CongestionControl::Block
        };
        Some(JanuBody::Unit(Unit {
            congestion_control,
            reply_context,
        }))
    }

    fn read_pull(&mut self, header: u8) -> Option<JanuBody> {
        let key = self.read_reskey(imsg::has_flag(header, zmsg::flag::K))?;
    ...
    fn read_declare(&mut self, _header: u8) -> Option<JanuBody> {
        let declarations = self.read_declarations()?;
        Some(JanuBody::Declare(Declare { declarations }))
    }
    ...
    fn read_query(&mut self, header: u8) -> Option<JanuBody> {
        let key = self.read_reskey(imsg::has_flag(header, zmsg::flag::K))?;
    ...
    fn read_link_state_list(&mut self, _header: u8) -> Option<JanuBody> {
        let len = self.read_zint()?;
        let mut link_states: Vec<LinkState> = Vec::with_capacity(len as usize);
        for _ in 0..len {
            link_states.push(self.read_link_state()?);
        }
        Some(JanuBody::LinkStateList(LinkStateList { link_states }))
    }
