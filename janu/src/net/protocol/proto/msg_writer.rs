...
impl WBuf {
        ...
        match &mut frame.payload {
            ...
            FramePayload::Messages { messages } => {
                for m in messages.iter_mut() {
                    zcheck!(self.write_janu_message(m));
                }
                true
            }
        }
    ...
    /*************************************/
    #[allow(clippy::let_and_return)]
    pub fn write_janu_message(&mut self, msg: &mut JanuMessage) -> bool {
        #[cfg(feature = "stats")]
        let start_written = self.len();
        ...
        let res = match &msg.body {
            JanuBody::Data(data) => self.write_data(data),
            JanuBody::Declare(declare) => self.write_declare(declare),
            JanuBody::Unit(unit) => self.write_unit(unit),
            JanuBody::Pull(pull) => self.write_pull(pull),
            JanuBody::Query(query) => self.write_query(query),
            JanuBody::LinkStateList(link_state_list) => {
