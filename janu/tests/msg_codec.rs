...
use janu::net::protocol::proto::defaults::SEQ_NUM_RES;
use janu::net::protocol::proto::*;
...
fn test_write_read_janu_message(mut msg: JanuMessage) {
    let mut buf = WBuf::new(164, false);
    println!("\nWrite message: {:?}", msg);
    buf.write_janu_message(&mut msg);
    println!("Read message from: {:?}", buf);
    let mut result = ZBuf::from(&buf)
        .read_janu_message(msg.channel.reliability)
        .unwrap();
    println!("Message read: {:?}", result);
...
fn codec_frame_batching() {
    for _ in 0..NUM_ITER {
        ...
        let mut data = JanuMessage::make_data(
            key,
            payload,
            channel,
            congestion_control,
        ...
        assert!(wbuf.write_janu_message(&mut data));
        ...
        let mut messages: Vec<JanuMessage> = vec![];
        loop {
            wbuf.mark();
            if wbuf.write_janu_message(&mut data) {
...
fn codec_declare() {
    for _ in 0..NUM_ITER {
        ...
        for roc in routing_context.iter() {
            for a in attachment.iter() {
                let msg = JanuMessage::make_declare(gen_declarations(), *roc, a.clone());
                test_write_read_janu_message(msg);
            }
        }
    }
}
        ...
        for ch in channel.iter() {
            for cc in congestion_control.iter() {
                for rc in reply_context.iter() {
                    for a in attachment.iter() {
                        let msg = JanuMessage::make_unit(*ch, *cc, rc.clone(), a.clone());
                        test_write_read_janu_message(msg);
                    }
                }
            }
    ...
    for _ in 0..NUM_ITER {
        ...
        for f in is_final.iter() {
            for m in max_samples.iter() {
                for a in attachment.iter() {
                    let msg = JanuMessage::make_pull(*f, gen_key(), gen!(ZInt), *m, a.clone());
                    test_write_read_janu_message(msg);
                }
            }
        }
    ...
    for _ in 0..NUM_ITER {
        ...
        for p in predicate.iter() {
            for t in target.iter() {
                for roc in routing_context.iter() {
                    for a in attachment.iter() {
                        let msg = JanuMessage::make_query(
                            gen_key(),
                            p.clone(),
                            gen!(ZInt),
                            t.clone(),
