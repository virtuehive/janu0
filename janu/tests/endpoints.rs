...
use janu::net::protocol::core::{whatami, PeerId};
...
fn endpoint_unix() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-0.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-1.sock");
    // Define the locators
    let endpoints: Vec<EndPoint> = vec![
        "unixsock-stream/janu-test-unix-socket-0.sock"
            .parse()
            .unwrap(),
        "unixsock-stream/janu-test-unix-socket-1.sock"
            .parse()
            .unwrap(),
    ];
    task::block_on(run(&endpoints));
    let _ = std::fs::remove_file("janu-test-unix-socket-0.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-1.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-0.sock.lock");
    let _ = std::fs::remove_file("janu-test-unix-socket-1.sock.lock");
}
...
fn endpoint_tcp_udp_unix() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-2.sock");
    // Define the locators
    let endpoints: Vec<EndPoint> = vec![
        ...
        "unixsock-stream/janu-test-unix-socket-2.sock"
            .parse()
            .unwrap(),
    ];
    task::block_on(run(&endpoints));
    let _ = std::fs::remove_file("janu-test-unix-socket-2.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-2.sock.lock");
}
...
fn endpoint_tcp_unix() {
    ...
    // Remove the file if it still exists
    let _ = std::fs::remove_file("janu-test-unix-socket-3.sock");
    // Define the locators
    let endpoints: Vec<EndPoint> = vec![
        "tcp/127.0.0.1:9451".parse().unwrap(),
        "tcp/[::1]:9451".parse().unwrap(),
        "unixsock-stream/janu-test-unix-socket-3.sock"
            .parse()
            .unwrap(),
    ];
    task::block_on(run(&endpoints));
    let _ = std::fs::remove_file("janu-test-unix-socket-3.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-3.sock.lock");
}
...
fn endpoint_udp_unix() {
    ...
    // Remove the file if it still exists
    let _ = std::fs::remove_file("janu-test-unix-socket-4.sock");
    // Define the locators
    let endpoints: Vec<EndPoint> = vec![
        "udp/127.0.0.1:9451".parse().unwrap(),
        "udp/[::1]:9451".parse().unwrap(),
        "unixsock-stream/janu-test-unix-socket-4.sock"
            .parse()
            .unwrap(),
    ];
    task::block_on(run(&endpoints));
    let _ = std::fs::remove_file("janu-test-unix-socket-4.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-4.sock.lock");
}

#[cfg(feature = "transport_tls")]
#[test]
fn endpoint_tls() {
    use janu::net::link::tls::config::*;
