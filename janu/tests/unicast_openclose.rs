...
fn openclose_unix_only() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-9.sock");
    let endpoint: EndPoint = "unixsock-stream/janu-test-unix-socket-9.sock"
        .parse()
        .unwrap();
    task::block_on(openclose_transport(&endpoint));
    let _ = std::fs::remove_file("janu-test-unix-socket-9.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-9.sock.lock");
}
