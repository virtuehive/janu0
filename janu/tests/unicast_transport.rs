...
fn transport_unicast_unix_only() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-5.sock");
    // Define the locator
    let endpoints: Vec<EndPoint> = vec!["unixsock-stream/janu-test-unix-socket-5.sock"
        .parse()
        .unwrap()];
    // Define the reliability and congestion control
    let channel = [
    ...
    task::block_on(run(&endpoints, &channel, &MSG_SIZE_ALL));
    let _ = std::fs::remove_file("janu-test-unix-socket-5.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-5.sock.lock");
}
...
fn transport_unicast_tcp_unix() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-6.sock");
    // Define the locator
    let endpoints: Vec<EndPoint> = vec![
        "tcp/127.0.0.1:10449".parse().unwrap(),
        "tcp/[::1]:10449".parse().unwrap(),
        "unixsock-stream/janu-test-unix-socket-6.sock"
            .parse()
            .unwrap(),
    ];
    // Define the reliability and congestion control
    ...
    task::block_on(run(&endpoints, &channel, &MSG_SIZE_ALL));
    let _ = std::fs::remove_file("janu-test-unix-socket-6.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-6.sock.lock");
}
...
fn transport_unicast_udp_unix() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-7.sock");
    // Define the locator
    let endpoints: Vec<EndPoint> = vec![
        "udp/127.0.0.1:10449".parse().unwrap(),
        "udp/[::1]:10449".parse().unwrap(),
        "unixsock-stream/janu-test-unix-socket-7.sock"
            .parse()
            .unwrap(),
    ];
    // Define the reliability and congestion control
    ...
    task::block_on(run(&endpoints, &channel, &MSG_SIZE_NOFRAG));
    let _ = std::fs::remove_file("janu-test-unix-socket-7.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-7.sock.lock");
}
...
fn transport_unicast_tcp_udp_unix() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-8.sock");
    // Define the locator
    let endpoints: Vec<EndPoint> = vec![
        ...
        "unixsock-stream/janu-test-unix-socket-8.sock"
            .parse()
            .unwrap(),
    ];
    // Define the reliability and congestion control
    ...
    task::block_on(run(&endpoints, &channel, &MSG_SIZE_NOFRAG));
    let _ = std::fs::remove_file("janu-test-unix-socket-8.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-8.sock.lock");
}
