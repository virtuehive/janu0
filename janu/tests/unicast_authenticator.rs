...
use janu::net::transport::unicast::authenticator::SharedMemoryAuthenticator;
use janu::net::transport::unicast::authenticator::UserPasswordAuthenticator;
...
fn authenticator_unix() {
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-10.sock");
    let endpoint: EndPoint = "unixsock-stream/janu-test-unix-socket-10.sock"
        .parse()
        .unwrap();
    task::block_on(async {
        authenticator_user_password(&endpoint).await;
    ...
    let _ = std::fs::remove_file("janu-test-unix-socket-10.sock");
    let _ = std::fs::remove_file("janu-test-unix-socket-10.sock.lock");
}
