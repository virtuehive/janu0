...
###
### Dockerfile building the Eclipse janu router (janud)
###
#
# To build this Docker image: 
#   docker build -t eclipse/janu .
#
# To run this Docker image (without UDP multicast scouting):
#    docker run --init -p 7447:7447/tcp -p 7447:7447/udp -p 8000:8000/tcp eclipse/janu
#
# To run this Docker image with UDP multicast scouting (Linux only):
#    docker run --init -net host eclipse/janu
...
COPY janu janu
COPY janu-ext janu-ext
COPY janu-util janu-util
...
COPY --from=builder target/x86_64-unknown-linux-musl/release/janud /
COPY --from=builder target/x86_64-unknown-linux-musl/release/*.so /

RUN echo '#!/bin/ash' > /entrypoint.sh
RUN echo 'echo " * Starting: /janud $*"' >> /entrypoint.sh
RUN echo 'exec /janud $*' >> /entrypoint.sh
