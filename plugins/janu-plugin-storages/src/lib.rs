...
use janu::net::runtime::Runtime;
use janu::{ChangeKind, Path, Properties, Selector, Value, ZError, ZErrorKind, ZResult, Janu};
use janu_backend_traits::{Backend, PROP_STORAGE_PATH_EXPR};
use janu_plugin_trait::prelude::*;
use janu_util::{zerror, LibLoader};
...
janu_plugin_trait::declare_plugin!(StoragesPlugin);
pub struct StoragesPlugin {}
impl Plugin for StoragesPlugin {
    fn compatibility() -> janu_plugin_trait::PluginId {
        janu_plugin_trait::PluginId {
            uid: "janu-plugin-storages",
        }
    }
...
async fn run(runtime: Runtime, args: ArgMatches<'_>) {
    ...
    let janu = Arc::new(Janu::init(runtime).await);
    let workspace = janu
    ...
    if !args.is_present("no-backend") {
        ...
        let handle = start_backend(mem_backend, mem_backend_path.clone(), janu.clone())
            .await
            .unwrap();
        ...
        unsafe {
            match lib.get::<CreateBackend>(CREATE_BACKEND_FN_NAME) {
                Ok(create_backend) => match create_backend(&props) {
                    Ok(backend) => start_backend(backend, path.clone(), janu).await,
                    Err(err) => zerror!(
                        ZErrorKind::Other {
                            descr: format!(
