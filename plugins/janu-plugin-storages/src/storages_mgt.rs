...
use janu::{Path, PathExpr, ZResult, Janu};
use janu_backend_traits::{IncomingDataInterceptor, OutgoingDataInterceptor, Query};

pub(crate) async fn start_storage(
    mut storage: Box<dyn janu_backend_traits::Storage>,
    admin_path: Path,
    path_expr: PathExpr,
        ...
        loop {
            select!(
                ...
                query = storage_queryable.receiver().next().fuse() => {
                    let q = query.unwrap();
                    // wrap janu::net::Query in janu_backend_traits::Query
